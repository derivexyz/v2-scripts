...
export async function tryRequest<T = any, R = any>(
  ...
  try {
    if (verbose) {
      logger.debug(`\x1b[34m${method} ${url}\x1b[0m request with data: \n ${JSON.stringify(data, null, '\t')}\n`);
    }
    ...
    if (error instanceof AxiosError) {
      errorMessage = `Caught Axios error while ${method}:${url} with message ${error.message} and code ${error.code}`;
