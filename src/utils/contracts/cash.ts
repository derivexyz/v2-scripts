...
export async function approveIfNotApproved(
  wallet: ethers.Wallet,
  tokenContract: string,
  approvedAddr: string,
  checkAmount: bigint,
  approveAmount: bigint = BigInt(2) ** BigInt(256) - BigInt(1),
) {
  const currentAllowance: bigint = await callWeb3(
    null,
    tokenContract,
    'allowance(address,address)',
    [wallet.address, approvedAddr],
    ['uint'],
  );
